...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild, ScrollingView {
    ...
    public NestedScrollView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    private int computeScrollDuration(int dy, float velocityY) {
        final int height = (getHeight() + getPaddingTop() + getPaddingBottom()) / PULLDOWN_SCALE;
        ...
        duration = Math.min(duration, ANIMATED_SCROLL_GAP);
        return duration;
    }
